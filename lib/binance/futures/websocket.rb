  ...
  class Futures
    ...
    class WebSocket < WebSocketBase
      ...
      def kline(symbol:, interval:, callbacks:)
        url = "#{@base_url}/ws/#{symbol.downcase}_#{contractType}@kline_#{interval}"
        create_connection(url, callbacks)
      end

      def continuous_kline(symbol:, interval:, contract_type:, callbacks:)
        url = "#{@base_url}/ws/#{symbol.downcase}_#{contractType}@continuousKline_#{interval}"
        create_connection(url, callbacks)
      end
